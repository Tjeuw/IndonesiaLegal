...
def detect_obligation_type(text):
    """
    Detect the obligation character of a chunk from its Indonesian keywords.
    Returns one of: mandatory | prohibited | permitted | conditional | declaratory
    Priority order: prohibited > mandatory > conditional > permitted > declaratory
    """
    t = text.lower()
    if any(w in t for w in ['dilarang', 'tidak boleh', 'tidak dapat', 'tidak diperkenankan']):
        return 'prohibited'
    if any(w in t for w in ['wajib', 'harus', 'berkewajiban', 'diwajibkan', 'wajib memiliki']):
        return 'mandatory'
    if any(w in t for w in ['dapat dikecualikan', 'dengan syarat', 'apabila', 'sepanjang', 'dalam hal']):
        return 'conditional'
    if any(w in t for w in ['dapat', 'berhak', 'diperbolehkan', 'diizinkan', 'boleh']):
        return 'permitted'
    return 'declaratory'
