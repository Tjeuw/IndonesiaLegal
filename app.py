...
def clean_ocr_text(text):
    """
    Clean common OCR artifacts from PDF-extracted legal text before chunking.
    Removes page numbers, running headers/footers, and fixes common substitutions.
    Does NOT alter the actual legal text content — only strips structural noise.
    """
    import re
    lines = text.split('\n')
    cleaned = []

    # Patterns that identify noise lines (not legal content)
    noise_patterns = [
        re.compile(r'^\s*-\s*\d+\s*-\s*$'),            # page markers: -130-
        re.compile(r'^\s*\d+\s*$'),                      # bare page numbers: 7, 8, 130
        re.compile(r'^\s*www\.\S+\s*$'),                 # URLs
        re.compile(r'^\s*https?://\S*\s*$'),             # URLs
        # Standalone headers that bleed from PDF layout
        re.compile(r'^\s*PRESIDEN\s*$'),
        re.compile(r'^\s*REPUBLIK\s+INDONESIA\s*$'),
        re.compile(r'^\s*REPUBUK\s+INDONESIA\s*$'),      # common OCR typo
        ...
        re.compile(r'^\s*jdih\.go\.id\s*$'),
        re.compile(r'^\s*jdih\.\w+\.go\.id\s*$'),
    ]

    for line in lines:
        stripped = line.strip()
        # Skip empty lines that create excessive whitespace (keep single blank lines)
        if not stripped:
    ...
    # Fix common OCR character substitutions in Indonesian legal terms
    ocr_fixes = [
        # "REPUBUK" → "REPUBLIK" (common l→L swap)
        (re.compile(r'\bREPUBUK\b'), 'REPUBLIK'),
        # Trailing noise digits after punctuation in legal refs: "Pasal 27 Ayat (3)7" → "Pasal 27 Ayat (3)"
