...
function parseCitations(html) {
  // Pass 1: full citations — (UU No. 6 Tahun 2023, Pasal 27 Ayat (3) [Level 3])
  const fullCitationRe = /\(([^)]*(?:Nomor|No\.)[^)]*Pasal[^)]*?)\)/gi;
  html = html.replace(fullCitationRe, (match, inner) => {
  ...
  // Pass 2: pasal-only citations — (Pasal 27 Ayat (1)) or (Pasal 3)
  // These appear when the AI references a pasal without repeating the full regulation name.
  // We pass the surrounding text via data attribute so the panel can attempt a lookup.
  const pasalOnlyRe = /\((Pasal\s+\d+(?:\s+Ayat\s*\(?\d+\)?)?)\)/gi;
  html = html.replace(pasalOnlyRe, (match, inner) => {
    // Skip if already wrapped by Pass 1
    if (match.includes('inline-citation')) return match;
    const encoded = encodeURIComponent(inner.trim());
    return `<span class="inline-citation pasal-only" onclick="openCitationPanel('${encoded}')" title="Click to view source text">${match}<span class="citation-icon">§</span></span>`;
  });

  // Style [General knowledge] tags
